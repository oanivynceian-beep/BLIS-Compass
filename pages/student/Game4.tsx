...
type Screen = 'START' | 'INSTRUCTIONS' | 'LEVEL_SELECT' | 'GAME' | 'RESULTS';
...
export default function ShelfShuffle() {
  ...
  const renderLevelSelect = () => (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#1a1c3a] text-white p-8">
      <h2 className="text-5xl font-bold mb-12 text-cyan-400 tracking-tight">SELECT LEVEL</h2>
      
      <div className="grid grid-cols-5 gap-4 max-w-2xl w-full">
        {Array.from({ length: 20 }, (_, i) => i + 1).map(id => {
          const isUnlocked = id === 1 || completedLevels.includes(id - 1) || SHELF_SHUFFLE_LEVELS.some(l => l.id === id);
          const isCompleted = completedLevels.includes(id);
          const hasData = SHELF_SHUFFLE_LEVELS.some(l => l.id === id);
    ...
    return (
      <div className="flex flex-col min-h-screen bg-[#1a1c3a] text-white">
        ...
        <div className="p-4 flex justify-between items-center bg-black/20 backdrop-blur-sm border-b border-white/10">
          ...
          <div className="relative">
            <div className="bg-[#d97706] px-6 py-2 rounded-sm border-2 border-[#92400e] shadow-lg">
              <span className="font-serif font-bold text-white">Shelf {currentLevelId}</span>
            </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 overflow-hidden">
          <div className="relative w-full max-w-5xl bg-[#f5e6d3] p-12 rounded-lg shadow-2xl border-x-8 border-b-8 border-[#d4b483]">
            {/* Shelf Background */}
            ...
            <Reorder.Group
              ...
              className="relative flex justify-center gap-2 h-80"
            >
              {shuffledBooks.map((book) => (
                <Reorder.Item
                  key={book.id}
                  value={book}
                  className="relative w-24 h-full cursor-grab active:cursor-grabbing"
                >
                  <div className="w-full h-full bg-gradient-to-b from-rose-500 to-rose-600 rounded-t-sm shadow-md flex flex-col justify-end">
                    {/* Spine Detail */}
                    <div className="absolute top-4 left-1/2 -translate-x-1/2 w-16 h-4/5 bg-rose-400/30 rounded-sm"></div>
                    
                    {/* Label */}
                    <div className="bg-white m-1 p-2 text-slate-900 text-[10px] leading-tight font-mono border border-slate-200">
                      {book.callNumber.prefix && <div>{book.callNumber.prefix}</div>}
                      <div>{book.callNumber.line1}</div>
                      <div>{book.callNumber.line2}</div>
                      <div>{book.callNumber.line3}</div>
                    </div>
                  </div>
                </Reorder.Item>
              ))}
  ...
  const renderResults = () => (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#1a1c3a] text-white p-8">
      ...
      >
        ...
        <h2 className="text-4xl font-bold mb-4">Level {currentLevelId} Complete!</h2>
        <p className="text-slate-300 mb-12">You've mastered the order of these books. Great job, Librarian!</p>

        <div className="flex flex-col gap-4">
          <button
            onClick={() => {
              const nextId = currentLevelId + 1;
              if (SHELF_SHUFFLE_LEVELS.some(l => l.id === nextId)) {
                startLevel(nextId);
              } else {
